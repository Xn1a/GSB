<?php
/**
 * Vue Liste des frais hors forfait éditables 
...
<div class="row">
        ...
        <table class="table table-bordered table-responsive">
            ...
            <tbody>
                ...
                ?>
                    <tr
                        <?php 
                        if (stripos($libelle, 'REFUSE :') !== false) { ?>
                    ...
                    <form method="post"
                        ...
                        <td><input class="form-control" type="text"
                                name="fraisHorsForfait[date]"
                                value="<?php echo $date; ?>"></td>
                        <td><input class="form-control" type="text"
                                name="fraisHorsForfait[libelle]"
                                value="<?php echo substr($libelle, 0, 26) ?>"></td>
                        <td><input class="form-control" type="text"
                                name="fraisHorsForfait[montant]"
                                value="<?php echo $montant; ?>"></td>
                        <td><input class="form-control" type="hidden"
                                name="fraisHorsForfait[estRefuse]"
                                value="<?php echo $estRefuse; ?>"></td>
                        <td>
                            <button class="btn btn-success"
                                type="submit">Corriger</button>
                            <button class="btn btn-info" type="submit"
                                name="reporter"
                                <?php if($idEtat == 'CR') { echo 'disabled'; } ?>
                                >Reporter
                            </button>
                            ...
                            <button class="btn btn-danger" type="submit"
                                name="refuser"
                                <?php if ($estRefuse || ($idEtat == 'CR')) { echo "disabled"; } ?>
                                >Refuser
                            </button>
                        </td>
                        <input type="hidden" name="fraisHorsForfait[id]"
                            value="<?php echo $id; ?>">
    ...
    <form method="post" action="index.php?uc=validerFiches&action=validerFiche"
        role="form">
        <input type="hidden" name="lstVisiteurs" value="<?php echo $idVisiteur; ?>">
        <input type="hidden" name="lstMois" value="<?php echo $leMois; ?>">
        <?php 
        if($idEtat == 'CR') { ?>
            <button class="btn btn-warning" type="submit" disabled>Saisie en
                cours...</button>
        <?php 
        } else { ?>
